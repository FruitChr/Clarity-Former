...
from .instance_fusion import DynamicMaskAffineFusion
from .neckaug import EASA, LDE

__all__ = ["ClarityFormer"]


@META_ARCH_REGISTRY.register()
class ClarityFormer(nn.Module):
    def __init__(self, cfg):
        super().__init__()

        self.device = torch.device(cfg.MODEL.DEVICE)

        self.scale_ranges = cfg.MODEL.CLARITY_FORMER.FEAT_SCALE_RANGES
        self.strides = cfg.MODEL.CLARITY_FORMER.FEAT_INSTANCE_STRIDES
        self.sigma = cfg.MODEL.CLARITY_FORMER.SIGMA
        self.num_classes = cfg.MODEL.CLARITY_FORMER.NUM_CLASSES
        self.num_kernels = cfg.MODEL.CLARITY_FORMER.NUM_KERNELS
        self.num_grids = cfg.MODEL.CLARITY_FORMER.NUM_GRIDS

        self.instance_in_features = cfg.MODEL.CLARITY_FORMER.INSTANCE_IN_FEATURES
        self.instance_strides = cfg.MODEL.CLARITY_FORMER.FEAT_INSTANCE_STRIDES
        self.instance_in_channels = cfg.MODEL.CLARITY_FORMER.INSTANCE_IN_CHANNELS
        self.instance_channels = cfg.MODEL.CLARITY_FORMER.INSTANCE_CHANNELS

        self.mask_on = cfg.MODEL.MASK_ON
        self.mask_in_features = cfg.MODEL.CLARITY_FORMER.MASK_IN_FEATURES
        self.mask_in_channels = cfg.MODEL.CLARITY_FORMER.MASK_IN_CHANNELS
        self.mask_channels = cfg.MODEL.CLARITY_FORMER.MASK_CHANNELS
        self.num_masks = cfg.MODEL.CLARITY_FORMER.NUM_MASKS

        self.resize_input_factor = cfg.MODEL.CLARITY_FORMER.RESIZE_INPUT_FACTOR
        # self.confidence_score = cfg.MODEL.CLARITY_FORMER.CONFIDENCE_SCORE
        self.max_before_nms = cfg.MODEL.CLARITY_FORMER.NMS_PRE
        self.score_threshold = cfg.MODEL.CLARITY_FORMER.SCORE_THR
        self.update_threshold = cfg.MODEL.CLARITY_FORMER.UPDATE_THR
        self.mask_threshold = cfg.MODEL.CLARITY_FORMER.MASK_THR
        self.max_per_img = cfg.MODEL.CLARITY_FORMER.MAX_PER_IMG
        self.nms_kernel = cfg.MODEL.CLARITY_FORMER.NMS_KERNEL
        self.nms_sigma = cfg.MODEL.CLARITY_FORMER.NMS_SIGMA
        self.nms_type = cfg.MODEL.CLARITY_FORMER.NMS_TYPE

        self.backbone = build_backbone(cfg)
        backbone_shape = self.backbone.output_shape()

        instance_shapes = [backbone_shape[f] for f in self.instance_in_features]
        self.cate_head = ClarityFormerTransformerHead(cfg, instance_shapes)

        mask_shapes = [backbone_shape['res' + f[-1]] for f in self.mask_in_features]
        self.mask_head = VGSEMaskHead(cfg, mask_shapes)

        self.ins_loss_weight = cfg.MODEL.CLARITY_FORMER.LOSS.DICE_WEIGHT
        self.focal_loss_alpha = cfg.MODEL.CLARITY_FORMER.LOSS.FOCAL_ALPHA
        self.focal_loss_gamma = cfg.MODEL.CLARITY_FORMER.LOSS.FOCAL_GAMMA
        self.focal_loss_weight = cfg.MODEL.CLARITY_FORMER.LOSS.FOCAL_WEIGHT
        ...
        self.no_fpn = cfg.MODEL.CLARITY_FORMER.NOFPN
        if self.no_fpn:
        ...
        self.sem_loss_on = cfg.MODEL.CLARITY_FORMER.SEM_LOSS
        self.sem_loss_weight = cfg.MODEL.CLARITY_FORMER.LOSS.SEM_WEIGHT
        self.sem_loss_type = cfg.MODEL.CLARITY_FORMER.LOSS.SEM_TYPE
        self.ins_edge_on = cfg.MODEL.CLARITY_FORMER.INS_EDGE
        self.ins_edge_weight = cfg.MODEL.CLARITY_FORMER.LOSS.INS_EDGE_WEIGHT
        self.ins_fusion = cfg.MODEL.CLARITY_FORMER.INS_FUSION
        self.dynamic_mask_fusion = DynamicMaskAffineFusion(
            self.num_kernels, cfg.MODEL.CLARITY_FORMER.DYNAMIC_MASK_NORM
        )
    ...
    # *** 鏂板杩欎釜鏂规硶 ***
    @torch.no_grad()
    def forward_with_attention_offsets(self, batched_inputs):
        """
        A dedicated forward pass to extract attention offsets from the encoder for visualization.
        Sets the model to eval mode and operates with no gradients.
        """
        self.eval() # 纭繚妯″瀷鍦ㄨ瘎浼版ā寮?        
        images = self.preprocess_image(batched_inputs)
        features = self.backbone(images.tensor)
        ...
        # 璋冪敤 ClarityFormerTransformerHead 涓柊澧炵殑鏂规硶
        ref_points, all_offsets, all_weights, spatial_shapes = self.cate_head.forward_with_attention_offsets(ins_features)
        
        return {
            "image": images.tensor,
            "reference_points": ref_points,
            "offsets": all_offsets[-1],
            "all_layer_offsets": all_offsets,
            "attention_weights": all_weights[-1],
    ...
    def loss(self, cate_preds, kernel_preds, ins_pred, targets, sem_targets=None, sem_pred=None):
        ...
        for b_kernel_pred in kernel_preds:
            ...
            for idx, kernel_pred in enumerate(b_kernel_pred):
                ...
                cur_ins_pred = self.dynamic_mask_fusion(cur_ins_pred, kernel_pred)
    ...
    def inference_single_image(
        ...
        # mask encoding.
        seg_preds = self.dynamic_mask_fusion(seg_preds, kernel_preds).sigmoid()
...
class ClarityFormerTransformerHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        """
        Clarity-Former transformer head.
        """
        super().__init__()
        ...
        self.hidden_dim = cfg.MODEL.CLARITY_FORMER.HIDDEN_DIM
        self.no_fpn = cfg.MODEL.CLARITY_FORMER.NOFPN
        if self.no_fpn:
        ...
        assert in_channels == cfg.MODEL.CLARITY_FORMER.INSTANCE_IN_CHANNELS, \
        ...
        prior_prob = cfg.MODEL.CLARITY_FORMER.PRIOR_PROB
        bias_value = -math.log((1 - prior_prob) / prior_prob)
        torch.nn.init.constant_(self.cate_pred.bias, bias_value)

        # query selection
        self.qs_enable = cfg.MODEL.CLARITY_FORMER.QS.ENABLE
        if self.qs_enable:
            self.qs_input = cfg.MODEL.CLARITY_FORMER.QS.INPUT
            self.qs_share_head = cfg.MODEL.CLARITY_FORMER.QS.SHARE_HEAD
            self.qs_nums = cfg.MODEL.CLARITY_FORMER.QS.NUM_QUERIES
    ...
    def forward(self, features):
        ...
        trans_memory = []
        memorys, level_start_index, *_ = self.trans_encoder(features, pos_encoder)
    ...
    def forward_with_attention_offsets(self, features):
        """
        A dedicated forward pass to extract attention offsets for visualization.
        """
        pos_encoder = []
        sizes_encoder = []
        for idx, feature in enumerate(features):
        ...
        memorys, level_start_index, spatial_shapes, ref_points, all_layer_offsets, all_layer_weights = self.trans_encoder(features, pos_encoder)
  
        return ref_points, all_layer_offsets, all_layer_weights, spatial_shapes

class VGSEMaskHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        """
        ...
        self.easa = EASA(dim=128)
        self.lde = LDE(dim=128)
        self.mask_on = cfg.MODEL.MASK_ON
        self.num_masks = cfg.MODEL.CLARITY_FORMER.NUM_MASKS
        self.mask_in_features = cfg.MODEL.CLARITY_FORMER.MASK_IN_FEATURES
        self.mask_in_channels = cfg.MODEL.CLARITY_FORMER.MASK_IN_CHANNELS
        ...
        norm = None if cfg.MODEL.CLARITY_FORMER.NORM == "none" else cfg.MODEL.CLARITY_FORMER.NORM
        ...
        self.sem_loss_on = cfg.MODEL.CLARITY_FORMER.SEM_LOSS
        self.single_sem = cfg.MODEL.CLARITY_FORMER.SINGLE_SEM
        if self.sem_loss_on:
    ...
    def forward(self, features):
        ...
        ### ========== Level 2 ==========
        feat_pre_level = self.convs_all_levels[2](features[2])
        feat_pre_level = self.easa(feat_pre_level)#娣诲姞easa
        ...
        if self.sem_loss_on and not self.single_sem:
            edge_preds.append(self.edge_all_levels[0](feature_add_all_level, feat_pre_level))

        feature_add_all_level = self.convs_all_sums[2](feature_add_all_level)

        ### ========== Level 1 ==========
        feat_pre_level = self.convs_all_levels[1](features[1])
        feat_pre_level = self.easa(feat_pre_level)#娣诲姞easa
        ...
        if self.sem_loss_on and not self.single_sem:
            edge_preds.append(self.edge_all_levels[1](feature_add_all_level, feat_pre_level))

        feature_add_all_level = self.convs_all_sums[1](feature_add_all_level)

        ### ========== Level 0 ==========
        feat_pre_level = self.convs_all_levels[0](features[0])
        feat_pre_level = self.lde(feat_pre_level)#娣诲姞easa

        feature_add_all_level += feat_pre_level
        feature_add_all_level = F.relu(feature_add_all_level, inplace=True)

        if self.sem_loss_on and not self.single_sem:
            edge_preds.append(self.edge_all_levels[2](feature_add_all_level, feat_pre_level))

        feature_add_all_level = self.convs_all_sums[0](feature_add_all_level)
