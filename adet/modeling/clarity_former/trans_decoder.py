...
class ClarityFormerTransformerDecoder(nn.Module):
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, src, pos_embed, memorys, pos_memory, reference_points, spatial_shapes,
        ...
        # 鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤鈻尖柤
        # 銆愪慨鏀圭偣銆戞鏌ヨ繑鍥炲€兼槸鍚︿负 tuple锛屽鏋滄槸锛屽彧鍙栫涓€涓厓绱?(Tensor)
        if isinstance(src2, tuple):
            src2 = src2[0]
        # 鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌鈻测柌
...
def build_transformer_decoder(cfg):
    return ClarityFormerTransformerDecoder(
        d_model=cfg.MODEL.CLARITY_FORMER.HIDDEN_DIM,
        nhead=cfg.MODEL.CLARITY_FORMER.NHEAD,
        num_encoder_layers=cfg.MODEL.CLARITY_FORMER.DEC_LAYERS,
