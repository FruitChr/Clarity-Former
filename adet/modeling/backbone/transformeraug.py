...
class ALOFTE(nn.Module):
    def __init__(self, dim, mask_radio=0.1, mask_alpha=0.5,
                 noise_mode=1, uncertainty_model=2, perturb_prob=0.5,
                 uncertainty_factor=1.0, noise_layer_flag=0, gauss_or_uniform=0):
        super().__init__()
    ...
    def _reparameterize(self, mu, std, epsilon_norm):
        return mu + epsilon_norm * std * self.factor

    def spectrum_noise(self, x, noise_mode=1, uncertainty_model=2, gauss_or_uniform=0):
        """
        x: [B, N, C] 输入特征
        """
        ...
        if noise_mode != 0:
            if uncertainty_model == 1:
                # batch-level
                mu = img.mean(dim=1, keepdim=True)  # [B, 1, C]
                var = ((img - mu) ** 2).mean(dim=1, keepdim=True)
                std = (var + self.eps).sqrt()  # [B, 1, C]

                mu_var = mu.var(dim=0, keepdim=True)
                std_var = std.var(dim=0, keepdim=True)
                std_mu = (mu_var + self.eps).sqrt().expand_as(mu)
                std_std = (std_var + self.eps).sqrt().expand_as(std)

                if gauss_or_uniform == 0:
                    eps_mu = torch.randn_like(std_mu)
                    eps_std = torch.randn_like(std_std)
                elif gauss_or_uniform == 1:
                    eps_mu = torch.rand_like(std_mu) * 2 - 1.
                    eps_std = torch.rand_like(std_std) * 2 - 1.
                else:
                    eps_mu = torch.randn_like(std_mu)
                    eps_std = torch.randn_like(std_std)

                beta = self._reparameterize(mu, std_mu, eps_mu)
                gamma = self._reparameterize(std, std_std, eps_std)
                beta = torch.clamp(beta, min=-1.0, max=1.0)
                gamma = torch.clamp(gamma, min=-1.0, max=1.0)

                img = gamma * (img - mu) / std + beta

            elif uncertainty_model == 2:
                # channel-wise
                mu = img.mean(dim=0, keepdim=True)  # [1, N, C]
                std = img.std(dim=0, keepdim=True)  # [1, N, C]

                if gauss_or_uniform == 0:
                    eps = torch.randn_like(img)
                elif gauss_or_uniform == 1:
                    eps = torch.rand_like(img) * 2 - 1.
                else:
                    eps = torch.randn_like(img)

                gamma = eps * std * self.factor
                img = img + gamma

            elif uncertainty_model == 3:
                # token-wise
                mu = img.mean(dim=2, keepdim=True)  # [B, N, 1]
                std = img.std(dim=2, keepdim=True)  # [B, N, 1]
    ...
    def forward(self, x, spatial_size=None):
        ...
        x: [B, N, C] 格式输入
        """
        if self.training and self.noise_mode != 0 and self.noise_layer_flag == 1:
            x = self.spectrum_noise(x, noise_mode=self.noise_mode,
...
class TransFormerAug(nn.Module):
    def __init__(self, dim, drop_path=0., h=8, norm_layer=nn.LayerNorm,):
        super().__init__()
        self.alofte = ALOFTE(dim=dim, uncertainty_model=2)
    ...
    def forward(self, input):
        ...
        x = input
        # print(f"Input shape: {x.shape}, expected norm dim: {self.norm1.normalized_shape}")
